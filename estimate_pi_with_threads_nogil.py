import threading
import random
import numpy as np #pi'nin gerçek değeri ve mutlak hesabı için 
from numba import jit #JIT derleyici, hesaplamaları hızlandırmak için kullanılır. nogil=True ile GIL devre dışı bırakılır.
...
class PiEstimatorThread(threading.Thread):
    def __init__(
        ...
        self.total_points = 0
'''
GIL ile yazılmış pi estiamtordan farkı burasıdır
@jit(nopython=True, nogil=True) sayesinde bu fonksiyon Dillerin babası olan C gibi çalışır
nogil=True parametresinin True set edilemsiyle GIL (Global Interpreter Lock) kullanılmaz, performans ciddi şekilde artar.
n tane nokta üretir, kaçı çember içindeyse onları sayar.
GIL ile yaptığımda belirtiğim gibi GIL yüzünden threading gerçek paralellik sağlamaz
ama numba ile JIT derlenmiş fı-onksiyonalr nogil=True ile GIL devre dışı kalır böylece
threading ile gerçekten parallel çalışan hesaplamalr yapılabilir
